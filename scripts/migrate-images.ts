...
async function main(): Promise<void> {
  ...
  for (const slot of IMAGE_REGISTRY) {
    ...
    if (!source) {
      reports.push({ slotId, status: "skipped-no-default" })
      continue
    }
    if (looksLikeOurBlob(source)) {
      reports.push({ slotId, status: "skipped-already-blob", sourceUrl: source })
      continue
    }
    ...
    try {
      ...
      reports.push({ slotId, status: "failed", sourceUrl: source, reason })
